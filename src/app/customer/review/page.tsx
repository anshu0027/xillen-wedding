...
export default function Review() {
  ...
  const guestRangeLabel =
    GUEST_RANGES.find((g) => g.value === state.maxGuests)?.label || "";
  ...
  const indoorOutdoorLabel =
    INDOOR_OUTDOOR_OPTIONS.find((o) => o.value === state.indoorOutdoor)
      ?.label || "";
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(12);
      doc.text(
        `Total Premium: ${formatCurrency(state.totalPremium)}`,
        pageWidth - 15,
        65,
        { align: "right" }
      );
      ...
      if (state.liabilityCoverage !== "none") {
        doc.text(
          `Liability Coverage: ${formatCurrency(state.liabilityPremium)}`,
          20,
          83
        );
      }
      if (state.liquorLiability) {
        doc.text(
          `Host Liquor Liability: ${formatCurrency(
            state.liquorLiabilityPremium
          )}`,
          20,
          91
        );
  ...
  const savePolicyAndPayment = useCallback(async () => {
    ...
    if (paymentSuccess && !policySaved) {
      ...
      try {
        console.log("Updating quote to COMPLETE:", {
          ...state,
          step: "COMPLETE",
        });

        // 1. Update quote to COMPLETE
        ...
        if (!quoteRes.ok) {
          console.error("Failed to update quote:", quoteData.error);
          alert(
            "Failed to update quote: " + (quoteData.error || "Unknown error")
          );
          setSavingPolicy(false);
          return;
        }
        ...
        if (quoteData.quote && quoteData.quote.quoteNumber) {
          try {
            ...
            if (convertRes.ok) {
              console.log(
                "Quote converted to policy successfully with policy number:",
              ...
              console.warn(
                "Failed to convert quote to policy:",
                convertData.error
              );
            }
          } catch (convertError) {
                      ...
                      <div className="space-y-8">
                        ...
                        <div className="text-sm text-gray-500 mt-6">
                          <p>
                            ...
                            policy documents at {state.email}. If you have any
                ...
                <>
                    ...
                    <div className="bg-yellow-50 border-l-4 border-yellow-400 rounded-lg p-4 mb-8 flex items-start gap-3">
                      ...
                      <div>
                        <p className="text-sm text-yellow-800 font-semibold">
                          ...
                          proceeding to payment. You can go back to make changes
                          if needed.
                        </p>
                      </div>
                    </div>
                  ...
                  >
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 w-full">
                      ...
                      <div>
                        <ReviewItem
                          label="Core Coverage"
                          value={coverageLevelLabel}
                        />
                        <ReviewItem
                          label="Liability Coverage"
                          value={liabilityOptionLabel}
                        />
                        <ReviewItem
                  ...
                  >
                    <div className="py-10 text-center">
                      <p className="text-gray-700 mb-4">
                        For this demonstration, we&apos;ve simplified the
                        payment process. Click the button below to simulate
                        payment and complete your policy purchase.
                      </p>
                      <Button
                        variant="primary"
                        size="lg"
                      ...
                      <p className="text-xs text-gray-500 mt-4">
                        ...
                        {formatCurrency(state.totalPremium)}. In a real
